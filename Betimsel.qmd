...
Bu bölümde, veri kümelerinin ve değişkenlerin temel özelliklerini ele alacağız. Verilerin görselleştirilmesinde yaygın olarak kullanılan grafik yöntemlerini inceledikten sonra, verileri sayısal olarak özetleyen betimsel istatistik araçlarını öğreneceğiz. Özellikle, merkezi eğilim (ortalama, medyan, mod) ve değişkenlik ölçüleri (standart sapma, varyans) üzerinde duracağız. Bu süreçte, verilerin dağılımını ve yapısını daha iyi anlamaya yönelik çeşitli yöntemler hakkında bilgi sahibi olacağız.
...
Anakütleden örnekleme ve istatistiksel çıkarım süreci 
...
İstatistiki Bölge Birimleri Sınıflaması (İBBS) Düzey 2 bölgeleri 
...
$$
olarak tanımlanır. Bir birey bir işte çalışmıyorsa, son dört hafta içerisinde aktif olarak iş arıyorsa ve iş bulduğu takdirde 2 hafta içinde işbaşı yapabilecek durumda ise işsiz olarak sınıflandırılır. İşgücü, "emek arzında bulunan çalışma
çağındaki" yani 15 ve yukarı yaştaki nüfusu kapsar. @fig-HIA HIA anketinde bu bilgilerle ilişkisi soruları göstermektedir (kaynak: İşgücü İstatistikleri Mikro Veri seti, TÜİK yayın no 4738, 2023). 
...
Hanehalkı İşgücü Araştırması örnek anket soruları (Kaynak: TÜİK) 

:::  

2023 yılında 15-64 yaş arası Türkiye geneli için işsizlik oranı %9.6 olarak tahmin edilmiştir. @fig-ibbs2unem 26 bölge için işsizlik oranlarını harita üzerinde göstermektedir. 

::: {#fig-ibbs2unem}

![](img/ibbs2unemp.png)

İBBS2 düzeyinde işsizlik oranları 
...
Deneysel veri süreci: rassal atama ile deney ve kontrol grupları
:::
...
digraph G {
  ...
  A [label = 'Rassal Atama']

  # Grup A ve izleyen akışın gri renkte olması
  node [shape = box, style=filled, fillcolor=lightgrey]
  B1 [label = 'Kontrol Grubu (Grup A)']
  C1 [label = 'Ölçüm']
  D1 [label = 'Müdahale/Tedavi Yok']
  E1 [label = 'Ölçüm']

  # Grup B ve izleyen akışın lightblue renkte olması
  node [shape = box, style=filled, fillcolor=lightblue]
  B2 [label = 'Deney Grubu (Grup B)']
  C2 [label = 'Ölçüm']
  D2 [label = 'Müdahale/Tedavi']
  E2 [label = 'Ölçüm']
...
::: {#exm-ts1}
`R`'da zaman serileri `ts` ya da `xts` nesnesi olarak tanımlanabilir. Bu konuda daha fazla detay için bkz. <https://htastan.github.io/Finansal-Ekonometri-MPI/R/R-zaman-serileri.html>

Zaman serilerini görselleştirmenin en kolay yolu x ekseninde zamanın ve y ekseninde ölçümlerin olduğu çizgi grafiğidir. @fig-bist100xts BIST 100 endeksinin 02/01/2003-29/09/2021 dönemine ait günlük kapanış değerlerini göstermektedir. 
```{r}
#| label: fig-bist100xts
#| fig-cap: "BIST100 endeksi günlük kapanış değerleri" 
#| echo: false
#| warning: false

library(xts)
load("Data/bist100.rda")
bistxts <- as.xts(bist100)   # xts nesnesine dönüştür
head(bistxts)
plot(bistxts$BIST100, main="")
```
...
Hanehalkı örnekleminde aylık geliri kullanarak yeni bir kategorik değişken oluşturalım. Aylık geliri 1883 TL'den düşük olanlar "Düşük Gelir", 1883-4230 arasında olanlar "Orta gelir" ve 4230 TL'den yüksek olanlar "Yüksek Gelir" olsun. 
```{r}
load("Data/hane_ornek.RData")
...
Aylık gelir düzeyi ile sigara arasındaki $2\times 3$ çapraz tabloyu oluşturalım:  
```{r}
...
: Gelir düzeyi ve sigara kullanımı {#tbl-capraztablo1} 

@tbl-capraztablo1 çapraz tabloyu göstermektedir. Toplam 2000 hane içinde 215'i düşük gelir grubundadır ve hanede sigara içilmektedir. Yüksek gelir grubundaki hanelerin 318'inde sigara içen vardır, 239 hanede ise yoktur. 
...
```
Bu tabloda satır toplamları 1'dir. Sigara içilen hanelerde orta gelirlilerin oranı yaklaşık %48.55'tir. 

```{r}
# sütun toplamına oran: 
prop.table(tablo, 2) * 100
```
Bu tabloda ise sütun toplamları 1'dir. Yüksek gelirli hanelerde sigara içilenlerin oranı %57.09, içilmeyenlerin oranı ise %42.91'dir. 


Çapraz tablolar çubuk çizimi ile görselleştirilebilir (bkz.  @fig-capraztablo1). 

```{r}
#| label: fig-capraztablo1
#| fig-cap: "Gelir grubuna göre sigara kullanımı"
#| warning: false
#| echo: true
# Yüzde hesaplama
...
Histogram ya da yoğunluk grafikleri dağılımın simetriklik, sağa ve sola çarpıklık gibi özelliklerine ilişkin önemli bilgiler verir. Simetrik bir dağılımın merkezinin iki tarafı birbirine benzer. @fig-simetri simetrik dağılan üç histogramı göstermektedir. Merkezden geçen kırmızı çizginin her iki tarafı benzer şekle sahiptir. 

```{r}
#| label: fig-simetri
#| fig-cap: "Dağılımların simetrikliği" 
#| echo: false 
par(mfrow=c(1,3))  # 3 histogramı yan yana yerleştir

# 1. Normal Dağılım
x <- rnorm(1000, mean=0, sd=1)  # Normal dağılım
hist(x, freq=FALSE, breaks=20, main="", xlab="", ylab="", col="lightblue", border="black")
abline(v=mean(x), col="red", lwd=2)  # Ortalama

# 2. Uniform Dağılım
x <- runif(10000, min=0, max=1)  # Düzgün (uniform) dağılım
hist(x, freq=FALSE, breaks=20, main="", xlab="", ylab="", col="lightgreen", border="black")
abline(v=mean(x), col="red", lwd=2)  # Ortalama

# 3. Simetrik Beta Dağılımı (Beta(2, 2))
x <- rbeta(10000, shape1=2, shape2=2)  # Simetrik beta dağılımı
hist(x, freq=FALSE, breaks=20, main="", xlab="", ylab="", col="lightcoral", border="black")
abline(v=mean(x), col="red", lwd=2)  # Ortalama

par(mfrow=c(1,1))  # Grafik yerleşimini eski haline döndür
...
```{r}
#| label: fig-asimetri
#| fig-cap: "Dağılımların çarpıklığı" 
#| echo: false 
par(mfrow=c(1,2))  # 2 histogramı yan yana yerleştir

# 1. Sağa Çarpık Dağılım (Gamma Dağılımı)
x <- rgamma(1000, shape=2, scale=2)  # Gamma dağılımı
hist(x, freq=FALSE, main="Sağa çarpık", xlab="", ylab="", col="lightblue", border="black")
abline(v=mean(x), col="red", lwd=2)  # Ortalama

# 2. Sola Çarpık Dağılım (Beta(2, 5))
x <- rbeta(1000, shape1=10, shape2=2)  # Beta dağılımı
hist(x, freq=FALSE, main="Sola çarpık", xlab="", ylab="", col="lightcoral", border="black")
abline(v=mean(x), col="red", lwd=2)  # Ortalama
...
`body.RData` vücut ölçümleri veri kümesini kullanarak aşağıdaki soruları yanıtlayınız:

  a) `height` (boy) değişkeninin kadın ve erkekler için ayrı ayrı kutu çizimlerini oluşturun ve özet istatistiklerle birlikte yorumlayın. 
  a) `height` ve `weight` değişkenlerinin serpilme çizimlerini oluşturun. Cinsiyete göre noktaları ayrıştırın (renk ya da geometrik şekil kullanılabilir, erkekler için kırmızı, kadınlar için mavi vb.).  
::: 
